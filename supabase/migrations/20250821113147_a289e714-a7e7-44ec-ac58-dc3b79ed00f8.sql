-- Add ads_manager role to users role enum
ALTER TYPE user_role ADD VALUE 'ads_manager';

-- Update job type constraint to include new ads manager job types  
ALTER TABLE jobs DROP CONSTRAINT IF EXISTS jobs_type_check;
ALTER TABLE jobs ADD CONSTRAINT jobs_type_check CHECK (type IN (
  'photo_session', 'product_photography', 'event_photography',
  'logo_design', 'branding', 'print_design', 'web_design', 'packaging_design', 
  'video_editing', 'photo_editing', 'color_correction', 'motion_graphics',
  'ads_plan', 'content_plan', 'visual_id', 'scripts'
));
